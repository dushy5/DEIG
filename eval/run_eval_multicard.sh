...
# ============================================
# 多卡并行推理脚本 - DEIG Bench 评估
# ============================================

# 默认配置参数
NUM_GPUS=${NUM_GPUS:-8}                    # GPU数量，默认4卡
OUTPUT_FOLDER=${OUTPUT_FOLDER:-"generation_samples/deigbench"}  # 输出目录
...
NEED_ATTRIBUTE_STATS=${NEED_ATTRIBUTE_STATS:-true}
MIOU_THRESHOLD=${MIOU_THRESHOLD:-0.5}
METRIC_NAME=${METRIC_NAME:-"deigbench"}
...
for ((i=0; i<${NUM_GPUS}; i++)); do
    ...
    # 设置CUDA可见设备并启动后台任务
    CUDA_VISIBLE_DEVICES=${i} python eval/eval_deigbench.py \
        ${BASE_ARGS} \
        --job_index ${i} \
...
if [ "${RUN_EVAL}" = true ]; then
    ...
    if [ "${NEED_ATTRIBUTE_STATS}" = true ]; then
        EVAL_ARGS="${EVAL_ARGS} --need_attribute_stats"
    fi
    
    EVAL_ARGS="${EVAL_ARGS} --miou_threshold ${MIOU_THRESHOLD} --metric_name ${METRIC_NAME}"
    
    echo "评估命令: python eval/eval_deigbench.py ${EVAL_ARGS}"
    echo ""
    
    # 在GPU 0上运行评估
    CUDA_VISIBLE_DEVICES=0 python eval/eval_deigbench.py ${EVAL_ARGS}
