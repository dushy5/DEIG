...
from ldm.modules.attention import DFMSpatialTransformer
from ldm.modules.core.util import (
    avg_pool_nd,
    conv_nd,
    linear,
    normalization,
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, masks):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, DFMSpatialTransformer):
                x = layer(x, context, objs, masks)
            else:
                x = layer(x)
        return x
...
class DEIGUNetModel(nn.Module):
    def __init__(
            ...
            IDE=None,
            mask_resolution_threshold=None,
    ):
        super().__init__()
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(DFMSpatialTransformer(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads,
        ...
        self.middle_block = TimestepEmbedSequential(
            ResBlock(ch,
                     ...
                     use_checkpoint=use_checkpoint,
                     use_scale_shift_norm=use_scale_shift_norm),
            DFMSpatialTransformer(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=dim_head,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if ds in attention_resolutions:
                    dim_head = ch // num_heads
                    layers.append(DFMSpatialTransformer(ch, key_dim=context_dim, value_dim=context_dim,
        ...
        self.IDE = instantiate_from_config(IDE)
    ...
    def forward(self, input):
        ...
        if self.training:
            combined_embeddings = grounding_input["object_positive_embeddings"]
            B, N, M, C = combined_embeddings.shape
            combined_processed = self.IDE(combined_embeddings.view(-1, M, C), input["timesteps"], caption=False)
            num_latent = combined_processed.shape[1]
            grounding_input["object_positive_embeddings"] = combined_processed.view(B, N, num_latent, 768)
            objs = self.position_net(**grounding_input)

        else:
            grounding_input_temp = deepcopy(grounding_input)
            combined_embeddings = grounding_input_temp["object_positive_embeddings"],
            combined_embeddings = combined_embeddings[0]
            B, N, M, C = combined_embeddings.shape
            combined_processed = self.IDE(combined_embeddings.view(-1, M, C), input["timesteps"], caption=False)
            num_latent = combined_processed.shape[1]
            grounding_input_temp["object_positive_embeddings"] = combined_processed.view(B, N, num_latent, 768)
            objs = self.position_net(**grounding_input_temp)



        # Grounding tokens: B*N*C
        #objs = self.position_net(**grounding_input)
        masks = grounding_input["inst_masks"]
        ...
        context = self.IDE(context, input["timesteps"], caption=True)

        # Start forwarding
        hs = []
        for module in self.input_blocks:
            if input["timesteps"][0] == 281:
                a = 5
            h = module(h, emb, context, objs, masks)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, masks)
