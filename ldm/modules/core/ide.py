...
class PerceiverAttentionBlock(nn.Module):
    def __init__(
        ...
        self.ln_0 = AdaLayerNorm(d_model, time_embedding_dim)
    ...
    def cross_attention(self, q: torch.Tensor, kv: torch.Tensor):
        attn_output, attn_output_weights = self.cross_attn(q, kv, kv, need_weights=False)
        return attn_output
    ...
    def forward(
        ...
        normed_latents = self.ln_0(latents, timestep_embedding)
...
class InstanceDetailExtractor(nn.Module):
    def __init__(
        self,
        time_channel=320,
        time_embed_dim=768,
    ...
    def forward(self, text_encode_features, timesteps, caption=False):
        ...
        B, N, D = ori_time_feature.shape
        ori_time_feature = ori_time_feature.unsqueeze(1).expand(-1, len(text_encode_features) // B, -1, -1).reshape(-1, N, D)
